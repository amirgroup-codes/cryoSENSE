# NeurIPS 2025 Submission - CryoSENSE Analysis Scripts
...
│   ├── cryogen_z.99.pkl           # Z-values for cryosense dataset
...
1. Download the required weight files from our [HuggingFace repository](https://huggingface.co/anon202628/CryoDRGN_model_weights/tree/main)
...
Generate 3D volumes for specific particles from the CryoSENSE, Original, and Low-res datasets (Figures 3c, f, i). Saves the resulting volumes to `cryodrgn_volumes/`:
...
Generate UMAP visualizations with GMM clustering for CryoSENSE, Low-res, and Original datasets (Figures 3b, e, h). Saves plots to `cryodrgn_plots/`. Saves correlation analysis between UMAP clusters to `cryodrgn_volumes/cluster_correlations.txt`:
...
Analyze and compare protein structures from ModelAngelo between CryoSENSE, Original, and Low-res datasets (Figures 4c-i). Saves results to `modelangelo_plots_outputs/`:
...
- CryoDRGN models were trained using the `cryodrgn train_vae` command on CryoSENSE, Original, and Low-res datasets
...
- The 3D volumes in Figure 2b can be found in our [HuggingFace repository](https://huggingface.co/datasets/anon202628/3D_Volumes_EMPIAR10076)
- The 3D volumes in Figure 4a can be found in our [HuggingFace repository](https://huggingface.co/datasets/anon202628/3D_Volumes_EMPIAR10648)
...
- The CIF files in `modelangelo_plots_data/` directory were generated by running ModelAngelo on:
  - Volumes from the CryoSENSE, Original, and Low-res reconstructions
