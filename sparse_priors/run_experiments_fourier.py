#!/usr/bin/env python3
"""
Run prior reconstruction experiments for CryoSENSE comparative methods.
...
priors = ["wavelet", "tv_minimize", "dct"]  # Options: 'wavelet', 'tv_minimize', 'dct'
...
def run_experiments():
    for exp in experiments:
        ...
        for block_size, mask_prob, mask_type in all_params:
            for prior in priors:
                print(f"  {prior} - block: {block_size}, prob: {mask_prob}, type: {mask_type}")
                for noise_level in noise_levels:
                    ...
                    if prior != 'dmplug':
                        grid_dir = os.path.join('grid_search', protein, prior,
                                                f"block_{block_size}_prob_{mask_prob}_{mask_type}")
