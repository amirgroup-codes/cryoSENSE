...
def get_reconstructed_indices(result_dir):
    """
    Scan the result directory and extract indices of already reconstructed images.
    Returns a set of reconstructed image indices.
    """
    reconstructed_indices = set()

    if not os.path.exists(result_dir):
        return reconstructed_indices

    # Look for reconstruction files matching the pattern: reconstruction_raw_image_*.pt
    import glob
    import re

    pattern = os.path.join(result_dir, 'reconstruction_raw_image_*.pt')
    reconstruction_files = glob.glob(pattern)

    # Extract indices from filenames
    for file_path in reconstruction_files:
        filename = os.path.basename(file_path)
        # Extract the number from reconstruction_raw_image_N.pt
        match = re.search(r'reconstruction_raw_image_(\d+)\.pt', filename)
        if match:
            idx = int(match.group(1))
            reconstructed_indices.add(idx)

    return reconstructed_indices

def get_missing_indices(total_images, reconstructed_indices):
    """
    Given the total number of images and the set of reconstructed indices,
    return a sorted list of missing indices that need to be reconstructed.
    """
    all_indices = set(range(total_images))
    missing_indices = sorted(all_indices - reconstructed_indices)
    return missing_indices

def group_into_ranges(indices):
    """
    Group a sorted list of indices into contiguous ranges.
    Returns a list of (start_id, end_id) tuples.
...
def analyze_results(result_dir, block_size, num_masks, mask_type, mask_prob):
    ...
    try:
        ...
        # Summary saving disabled - only reconstructed images will be saved
...
def main():
    parser = argparse.ArgumentParser(description='Run CryoSENSE Fourier masking reconstruction with varying mask probabilities')
    ...
    for experiment in experiments_to_run:
        ...
        for mask_prob in mask_probs:
            ...
            # Check for already reconstructed images
            reconstructed_indices = get_reconstructed_indices(mask_prob_result_dir)
            print(f"Found {len(reconstructed_indices)} already reconstructed images")

            if reconstructed_indices:
                # Show a sample of reconstructed indices
                sample_indices = sorted(list(reconstructed_indices))[:10]
                if len(reconstructed_indices) > 10:
                    print(f"Sample reconstructed indices: {sample_indices}... (and {len(reconstructed_indices) - 10} more)")
                else:
                    print(f"Reconstructed indices: {sample_indices}")

            # Get missing indices that need to be reconstructed
            missing_indices = get_missing_indices(total_images, reconstructed_indices)

            if not missing_indices:
                print(f"All {total_images} images already reconstructed for mask_prob={mask_prob}. Skipping.")
                continue

            print(f"Need to reconstruct {len(missing_indices)} remaining images")

            # Group missing indices into contiguous ranges
            missing_ranges = group_into_ranges(missing_indices)
            print(f"Grouped into {len(missing_ranges)} contiguous range(s)")

            # Distribute ranges across GPUs
            # Strategy: round-robin assignment of ranges to GPUs
            gpu_assignments = [[] for _ in gpu_ids]  # List of ranges for each GPU

            for idx, range_tuple in enumerate(missing_ranges):
                gpu_idx = idx % len(gpu_ids)
                gpu_assignments[gpu_idx].append(range_tuple)

            # Launch one process per GPU per range
            processes = []
            for gpu_idx, gpu_id in enumerate(gpu_ids):
                ranges_for_this_gpu = gpu_assignments[gpu_idx]

                if not ranges_for_this_gpu:
                    continue

                for start_id, end_id in ranges_for_this_gpu:
                    print(f"Assigning GPU {gpu_id}: images {start_id}-{end_id}")

                    # Create a separate process for each GPU job
        ...
        print("\nExpected trend: Higher mask_prob -> Better reconstruction quality")
